...
public class EvaluationQuestionController {
    ...
    public List<EvaluationQuestionDTO> getByEvaluation(@PathVariable Long evaluationId) {
        return service.getByEvaluation(evaluationId).stream().map(service::convertToDTO).toList();
    }

    // Body: { "evaluationId": 1, "questionId": 2, "points": 2.0, "ordre": 1 }
    @PostMapping
    public EvaluationQuestionDTO ajouter(@RequestBody Map<String, Object> body) {
        ...
        return service.convertToDTO(service.ajouterQuestion(evaluationId, questionId, points, ordre));
    }

    @PutMapping("/{id}")
    public ResponseEntity<EvaluationQuestionDTO> update(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        try {
            Double points = body.get("points") != null ? Double.valueOf(body.get("points").toString()) : null;
            Integer ordre = body.get("ordre") != null ? Integer.valueOf(body.get("ordre").toString()) : null;
            return ResponseEntity.ok(service.convertToDTO(service.update(id, points, ordre)));
